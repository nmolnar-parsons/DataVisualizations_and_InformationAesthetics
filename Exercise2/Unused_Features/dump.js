data_tv = d3.csv("Data/NetflixTV_added.csv").then( function(data_tv) {
    console.log(data_tv);
    console.log(typeof(data_tv[0]['Runtime']));
    ...
    data_tv.forEach ( d => {
        d.MinuteTime = timeStringToMinutes(d.Runtime);
        d.Views = +d.Views;
        d.genres = d.genres.split(","); // split genres into array

    })
    console.log(data_tv);
    console.log((data_tv[0]['genres'][1]));
    ...
    //     .domain(d3.extent(data_tv, d => d.Views)).nice()
    //     .range([height - marginBottom, marginTop]);


    // alternate fixed viewcount so we can observe more of the data
    y_axis = d3.scaleLinear()
        .domain([1000000,80000000]).nice()
        .range([height - marginBottom, marginTop]);


    //genres check
    MoviePerGenre = d3.rollup(data_tv, v => v.length, d => d.genres[0]); //naive as it only takes the first genre
    ...
    data_tv.forEach(d => d.genres.forEach(g => allGenres.add(g)));
